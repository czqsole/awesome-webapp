...
def has_request_args(fn):
    ...
    for name, param in params.items():
        if name == 'request':
            found = True
            continue
        if found and (param.kinf != inspect.Parameter.VAR_POSITIONAL and
...
def add_route(app, fn):
    ...
    app.router.add_route(method, path, RequestHandler(app, fn))  #here : the add_route is biutin


def add_routes(app, module_name):
    n = module_name.rfind('.')
    if n == -1:
        mod = __import__(module_name, globals(), locals())  # __import__ : dymaticed get func, mod = module
    else:
